...
from RD_to_postal_code_api import check_another_postalcode5
...
dire = os.getcwd()
input_file = "postcode5_number3.json"
output_file = "postcode5_3_normal.pkl"
print(output_file)
# Time variables
total = 0
count = 0
...
def sample_random_houses_close(data):
    ...
    try:
        ...
        for tries in range(3):
            ...
            if tries > 0:
                print("another try")
                da = HM.sample_in_range(rdx, rdy, 1000, 100, n)
            if len(da) == 0 and tries == 2:
                print("No samples are being returned")
            
            for home in da:
                home = check_another_postalcode5(home[0], home[1], origin_postcode)
                if home is not None:
